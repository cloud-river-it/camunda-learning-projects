...
public class ActivityLogService {
  ...
  private static String path = "/Users/fanida/Documents/CLOUD-RIVER/PROJECTS/poc-activitylog-projects/logs/activity-logs.log";
    ...
    Runnable runnableTask = () -> {
      try {
        while (twoSecondsLater.isAfter(LocalDateTime.now())) {
          ...
          Files.write(Paths.get(path), (out).getBytes(StandardCharsets.UTF_8),
   ...
   */
  public CompletableFuture<Void> correlateMessage(final ResponseCorrelation body) throws IOException, URISyntaxException {
    String requestBody = new ObjectMapper().writerWithDefaultPrettyPrinter().writeValueAsString(body);

    HttpRequest request = HttpRequest.newBuilder(new URI("http://localhost:8085/engine-rest/message"))
  ...
  private ResponseCorrelation buildCorrelateMessage(final String businessKey, final String taskId) {
    ResponseCorrelation response = new ResponseCorrelation(businessKey, businessKey.concat("-").concat(taskId));
